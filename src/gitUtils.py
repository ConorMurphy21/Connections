...
def save_to_git(args, user_config, file):
    ...
    if args.generate:
        subprocess.call(['git', 'commit', '-qm', f'{user_config.username} generated new game!'], cwd=DATA_PATH)
    else:
        subprocess.call(['git', 'commit', '-qm', f'{user_config.username} played a game!'], cwd=DATA_PATH)

    subprocess.run(['git', 'push'], cwd=DATA_PATH)
    result = subprocess.run(['git', 'status'], cwd=DATA_PATH, check_output=True, text=True).stdout
